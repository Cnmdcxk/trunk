    ...
    <div class="w1200">
        ...
        <div class="right_con" id="RZ" v-cloak>
            <div class="right_top">
                ...
                <span class="c_blue fl ml10">商品修改审核</span>
            </div>

            <div class="mt5">
                <div class="bd_t_dedede">
                    <ul class="menu_right supplier_menu">
                        <li><a href="/mall/goodsUpdate" style="cursor: pointer">待我审核（{{needMyCount}}）</a></li>
                            ...
                            <el-table-column prop="date" label="操作">
                                <template slot-scope="scope">
                                    <div>
                                        ...
                                        <button v-if="scope.row.status!= 'ZZSHBH' && scope.row.status!='ZYSHBH'" class="blue_txt_btn" @click="lookApplyReason1(scope.row.rejectreason,scope.row.audituser,scope.row.auditusername)">查看审批意见</button>
                                    </div>
                                    <div v-if="scope.row.status == 'ZZSHBH' || scope.row.status=='ZYSHBH'">
...
<@override name="js">
    ...
    <script type="text/babel" src="${cdn}/js/mall/myGoodsUpdate.js?ver=${ver!}"></script>
</@override>
