    ...
    <div class="w1200">
        ...
        <div class="right_con" id="RZ" v-cloak>
            <div class="right_top">
                ...
                <span class="c_blue fl ml10">商品挂牌审核(组长)</span>
            </div>

            <div class="mt5">
                <div class="bd_t_dedede">
                    <ul class="menu_right supplier_menu">
                        <li><a href="/mall/groupGoodsAudit" style="cursor: pointer">待我审核（{{needMyCount}}）</a></li>
                    ...
                    <div class="p10">
                        ...
                        <el-table :data="tableData" class="table_main_style" ref="multipleTable" @selection-change="handleSelectionChange">
                            ...
                            <el-table-column prop="date" label="操作">
                                <template slot-scope="scope">
                                    <div><button class="blue_txt_btn" @click="toPreview(scope.row.goodid)">预览</button> </div>
                                    <div v-if="scope.row.status == 'CGSHBH' || scope.row.status=='YBH'"><button class="blue_txt_btn" @click="openReDialog(scope.row.rejectreason,scope.row.audituser,scope.row.auditusername)">驳回原因</button></div>
                                    <button v-if="scope.row.status!='YBH'" class="blue_txt_btn" @click="lookApplyReason1(scope.row.rejectreason,scope.row.audituser,scope.row.auditusername)">查看审批意见</button>
                                </template>
                            </el-table-column>
                        </el-table>
                        <!-- 分页 -->
            ...
            <el-dialog title="查看驳回原因" :visible.sync="showReDialog" width="340px" :close-on-click-modal="false" center>
                <div class="">
                    <div class="pb10">{{auditUser}} {{updateuser}}  的审批意见</div>
                    <div class="mt10 w280">
...
<@override name="js">
    ...
    <script type="text/babel" src="${cdn}/js/mall/groupMyGoodsAudit.js?ver=${ver!}"></script>
</@override>
