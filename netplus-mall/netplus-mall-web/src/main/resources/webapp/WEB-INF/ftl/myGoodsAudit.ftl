...
<div class="w1200">
    ...
    <div class="right_con" id="RZ" v-cloak>
        <div class="right_top">
            ...
            <span class="c_blue fl ml10">商品挂牌审核</span>
        </div>

        <div class="mt5">
            <div class="bd_t_dedede">
                <ul class="menu_right supplier_menu">
                    <li><a href="/mall/goodsAudit" style="cursor: pointer">待我审核（{{needMyCount}}）</a></li>
                    ...
                    <el-table :data="tableData" class="table_main_style" ref="multipleTable" @selection-change="handleSelectionChange">
                        ...
                        <el-table-column prop="date" label="操作">
                            <template slot-scope="scope">
                                ...
                                <div v-if="scope.row.status != 'CGSHZ' ">
                                    <button class="blue_txt_btn" @click="lookApplyReason1(scope.row.rejectreason, scope.row.audituser, scope.row.auditusername)">查看审批意见</button>
                                </div>
                                <div v-if="scope.row.status == 'CGSHBH' || scope.row.status=='YBH'">
                                    <button class="blue_txt_btn" @click="openReDialog(scope.row.rejectreason,scope.row.audituser,scope.row.auditusername)">驳回原因</button>
                                </div>
                            </template>
                        </el-table-column>
                    </el-table>
...
<script type="text/babel" src="${cdn}/js/mall/myGoodsAudit.js?ver=${ver!}"></script>
</@override>
